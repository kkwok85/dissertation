...
demographic.var <- c("+ age + race + sex + marital.status + factor(disability)")
...
current.sit <- c("+ factor(current.situation) + factor(reason.not.work) + factor(reason.absent.last.week) + factor(enrolled.school)")
...
full.model <- lm(paste0(yvariables.v2[1]," ~ wfh.v3", demographic.var,employ.var, current.sit,family.bus, family.var, time.var, location.var, spouse.var, family.inc.var,  childcare.service.var ), data = combine.data.regress.v2, weights = tufnwgtp )
...
for (i in 1:length(yvariables.v2) ) {
  ...
  full.model <- lm(paste0(yvariables.v2[i]," ~ wfh.v3", demographic.var,employ.var, current.sit,family.bus, family.var, time.var, location.var, spouse.var, family.inc.var,  childcare.service.var ), data = combine.data.regress.v2, weights = tufnwgtp )
  result[[i]] <- full.model
...
stargazer(result[[1]], result[[2]],result[[3]], result[[4]], result[[5]], result[[6]], result[[7]],
          ...
          keep= c("wfh.v3" ), covariate.labels =c("Work from home(dummy version 1)","Unemployed/Not in labor force(dummy)"), out = "D:/ATUS_result_R/result_wfh.txt",
          notes = "Sampling weights are applied and robust standard errors are used. Base category is working at other places such as workplace. Number of observations is 22158.", notes.align = "l", omit.table.layout = "s" )
...
stargazer(result[[8]], result[[9]], result[[10]], result[[11]], result[[12]], result[[13]], result[[14]],
          ...
          keep= c("wfh.v3" ), covariate.labels =c("Work from home(dummy version 1)","Unemployed/Not in labor force(dummy)"), out = "D:/ATUS_result_R/result_wfh2.txt",
          notes = "Sampling weights are applied and robust standard errors are used. Base category is working at other places such as workplace. Number of observations is 22158.", notes.align = "l", omit.table.layout = "s" )
...
demographic.no.sex.var <- c("+ age + race  + marital.status + factor(disability)") 



for (i in 1:length(yvariables.v2) ) { 
  dep.var <- yvariables.v2[i]
  full.model <- lm(paste0(yvariables.v2[i]," ~ wfh.v3", demographic.no.sex.var ,employ.var, current.sit,family.bus, family.var, time.var, location.var, spouse.var, family.inc.var,  childcare.service.var ), data = combine.data.regress.v2.male , weights = tufnwgtp )
  result[[i]] <- full.model 
  robust.result[[i]] <- sqrt(diag(vcovHC(full.model, type = "HC1"))) 
} 
...
stargazer(result[[1]], result[[2]],result[[3]], result[[4]], result[[5]], result[[6]], result[[7]],
          ...
          notes = "Sampling weights are applied and robust standard errors are used. Base category is working at other places such as workplace. Number of observations in the dataset with male or female only is 10546 and 11612 respectively.", notes.align = "l", omit.table.layout = "s" )
...
for (i in 1:length(yvariables.v2) ) { 
  dep.var <- yvariables.v2[i]
  full.model <- lm(paste0(yvariables.v2[i]," ~ wfh.v3", demographic.no.sex.var ,employ.var, current.sit,family.bus, family.var, time.var, location.var, spouse.var, family.inc.var,  childcare.service.var ), data = combine.data.regress.v2.female , weights = tufnwgtp )
  result[[i]] <- full.model 
  robust.result[[i]] <- sqrt(diag(vcovHC(full.model, type = "HC1"))) 
} 
