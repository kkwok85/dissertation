...
for (i in 1:length(yvariables.v3)) {    
  ...
  a <- svyby( ~get(yvariables.agg[i]),~sex, z, svymean, na.rm = TRUE)
  b <- svyby( ~get(yvariables.agg[i]),~sex, z, svyvar, na.rm = TRUE)
  
  c <- svymean( ~get(yvariables.agg[i]) , z , na.rm = TRUE)
  d <- sqrt(svyvar( ~get(yvariables.agg[i]) , z , na.rm = TRUE))
  ...
  empty.frame[row[i],2] <- names.y.agg[i]
  empty.frame[row[i],3] <- signif(c, digits=3)
  empty.frame[row[i]+1,3] <- paste0( "(",signif(d, digits = 3),")")
...
xvariables <- c("total.job.wfh.time", "age", "race.edit1", "race.edit2", "race.edit3", "race.edit4", "race.edit5" ,"sexMale", "sexFemale",
                ...
                "edited.employ.statusEmployed","edited.employ.statusNot employed", "work.hours", 
...
names <- c("Work from home minutes", "Age", "1 if non-Hispanic White", "1 if non-Hispanic black", "1 if Hispanic","1 if non-Hispanic Asian","1 if others", "1 if male", "1 if female",
           ...
           "1 if Employed", "1 if not employed", "Weekly work hours*", 
