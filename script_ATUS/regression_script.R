...
stargazer(result[[1]], result[[2]],result[[3]], result[[4]], result[[5]], result[[6]], result[[7]], result[[8]], result[[9]], result[[10]], result[[11]], result[[12]], result[[13]], result[[14]],
          ...
          keep= c("wfh_mom" ), covariate.labels =c("Work from home(dummy)","Unemployed/Not in labor force(dummy)"),  type="latex", out = "D:/ATUS_result_R/result_wfh_mom.tex")
...
stargazer(result[[1]], result[[2]],result[[3]], result[[4]], result[[5]], result[[6]], result[[7]], result[[8]], result[[9]], result[[10]], result[[11]], result[[12]], result[[13]], result[[14]],
          ...
          column.labels=c(yvariables[1],yvariables[2], yvariables[3], yvariables[4], yvariables[5], yvariables[6], yvariables[7], yvariables[8], yvariables[9], yvariables[10],
                          yvariables[11], yvariables[12], yvariables[13], yvariables[14]), 
          keep= c("wfh_mom.v2" ), covariate.labels =c("Work from home(dummy)","Unemployed/Not in labor force(dummy)") , type="latex", out = "D:/ATUS_result_R/result_wfh_mom.v2.tex")
...
# controls slightly changed!working hours is dropped!
...
for (i in 1:length(yvariables) ) {
  dep.var <- yvariables[i]
  full.model <- lm(paste0(yvariables[i]," ~ edited.employ.status", demographic.var, "+ edited.occupations + factor(edited.occupations.indicator)",family.var, time.var, location.var, spouse.var, family.inc.var,  childcare.service.var ), data = combine.data.regress, weights = tufnwgtp )
  result[[i]] <- full.model
  robust.result[[i]] <- sqrt(diag(vcovHC(full.model, type = "HC1"))) 
}
